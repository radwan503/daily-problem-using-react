...
import Highcharts from 'highcharts/highmaps'; // Import Highmaps directly
import { Options } from 'highcharts';


const MapComponentsHighChart: React.FC = () => {
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const options: Options = {
          ...
          subtitle: {
            text:
              'Source map: <a href="http://code.highcharts.com/mapdata/countries/bd/bd-all.topo.json">Bangladesh</a>',
          },
          mapNavigation: {
            enabled: true,
            buttonOptions: {
          ...
          tooltip: {
            formatter: function () {
              const pointData = this.point as any;
              const crops = pointData.crops || []; // Ensure crops array exists
              return `<b>${pointData.name}</b><br>${crops.join(', ')}`;
            },
          },
          series: [
            {
...
export default MapComponentsHighChart;
